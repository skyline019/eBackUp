...
Status HashRegionsWithDigestBase(DigestAlgo algo, const uint8_t* digest_base,
  ...
  if (digest_base == nullptr) {
    return Status::InvalidArgument(
        "TopoCDC streaming requires digest_base; no copy fallback");
  }
...
void SetCarryTail(TopoCdcStreamState* state, const uint8_t* data, size_t consumed,
                  size_t total, TopoCdcStreamProfile* profile) {
  if (consumed >= total) {
    state->carry.clear();
    return;
  }
  ...
  if (state->hom_resume.in_scan && state->hom_resume.scan_rel >= consumed) {
    state->hom_resume.scan_rel -= consumed;
  }
  if (state->tri_resume.in_scan && state->tri_resume.scan_rel >= consumed) {
    state->tri_resume.scan_rel -= consumed;
  }
}

Status ProcessTopoFeedBuffer(TopoCdcStreamState* state, const uint8_t* proc,
  ...
  if (cfg.variant != TopoCdcVariant::kHom &&
      cfg.variant != TopoCdcVariant::kTri) {
    return Status::Internal("TopoCDC streaming supports Hom and Tri only");
  }

  const bool is_tri = cfg.variant == TopoCdcVariant::kTri;

  const bool profile_enabled = StreamProfileEnabled();
  TopoCdcStreamProfile* profile = profile_enabled ? &state->profile : nullptr;
  ...
  if (proc_len <= cfg.min_size) {
    ...
    if (is_tri) {
      topo_tri_internal::ClearTopoTriResume(&state->tri_resume);
    } else {
      topo_cdc_internal::ClearTopoHomResume(&state->hom_resume);
    }
    return Status::Ok();
  }
  ...
  size_t pos = 0;
  {
    ...
    uint64_t* probes =
        profile ? &profile->topo_scan_probes : nullptr;
    while (pos < proc_len) {
      size_t cut = 0;
      bool chunk_done = false;
      if (is_tri) {
        topo_tri_internal::ProcessTriChunk(
            proc, proc_len, pos, false, cfg, mask, state->gear,
            &state->tri_resume, &cut, &chunk_done, probes);
      } else {
        topo_cdc_internal::ProcessHomChunk(
            proc, proc_len, pos, false, cfg, mask, state->gear,
            &state->hom_resume, &cut, &chunk_done, probes);
      }
      if (!chunk_done) break;
  ...
  if (is_last && !state->carry.empty()) {
    ...
    if (is_tri) {
      topo_tri_internal::ProcessTriChunk(
          tail, tail_len, 0, true, cfg, mask, state->gear, &state->tri_resume,
          &cut, &chunk_done, &probes);
    } else {
      topo_cdc_internal::ProcessHomChunk(
          tail, tail_len, 0, true, cfg, mask, state->gear, &state->hom_resume,
          &cut, &chunk_done, &probes);
    }
...
void TopoCdcStreamInit(TopoCdcStreamState* state, TopoCdcConfig config) {
  ...
  ResetTopoCdcStreamProfile(&state->profile);
  topo_cdc_internal::ClearTopoHomResume(&state->hom_resume);
  topo_tri_internal::ClearTopoTriResume(&state->tri_resume);
  if (config.variant == TopoCdcVariant::kHom ||
      config.variant == TopoCdcVariant::kTri) {
    topo_cdc_internal::InitGearTable(state->gear, config.table_seed);
  }
...
Status TopoCdcStreamFeed(TopoCdcStreamState* state, const uint8_t* data,
  ...
  if (!state->tables_ready) TopoCdcStreamInit(state, state->config);
  ...
  return ProcessTopoFeedBuffer(state, proc, proc_len, is_last, out_chunks);
}
