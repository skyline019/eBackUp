...
void Sha256Core(const uint8_t* data, size_t len, uint8_t hash_out[32]) {
  Sha256Ctx ctx;
  Sha256Init(&ctx);
...
void HmacSha256Core(const uint8_t* key, size_t key_len, const uint8_t* data,
                    size_t data_len, uint8_t out[32]) {
  uint8_t k[64]{};
  if (key_len > 64) {
    Sha256Core(key, key_len, k);
    key_len = 32;
    key = k;
  } else {
...
void Pbkdf2Sha256Core(const uint8_t* password, size_t password_len,
                      const uint8_t* salt, size_t salt_len, uint32_t iterations,
                      size_t dk_len, uint8_t* out) {
  const size_t blocks = (dk_len + 31) / 32;
  for (size_t block = 1; block <= blocks; ++block) {
    uint8_t salt_block[128];
    if (salt_len + 4 > sizeof(salt_block)) return;
    std::memcpy(salt_block, salt, salt_len);
    salt_block[salt_len] = static_cast<uint8_t>((block >> 24) & 0xFF);
    salt_block[salt_len + 1] = static_cast<uint8_t>((block >> 16) & 0xFF);
    salt_block[salt_len + 2] = static_cast<uint8_t>((block >> 8) & 0xFF);
    salt_block[salt_len + 3] = static_cast<uint8_t>(block & 0xFF);

    uint8_t u[32];
    HmacSha256Core(password, password_len, salt_block, salt_len + 4, u);
    uint8_t t[32];
    std::memcpy(t, u, 32);
    for (uint32_t i = 1; i < iterations; ++i) {
      HmacSha256Core(password, password_len, u, 32, u);
      for (int j = 0; j < 32; ++j) t[j] ^= u[j];
    }
    const size_t offset = (block - 1) * 32;
    const size_t copy_len = (offset + 32 <= dk_len) ? 32 : (dk_len - offset);
