#include "ebbackup/crypto/aes_block.h"

#include <cstring>

namespace ebbackup {
namespace crypto {

namespace {

constexpr uint8_t kSbox[256] = {
    };

constexpr uint32_t kRcon[8] = {0x01000000, 0x02000000, 0x04000000, 0x08000000,
                               0x10000000, 0x20000000, 0x40000000, 0x80000000};

inline uint32_t SubWord(uint32_t w) {
  return (static_cast<uint32_t>(kSbox[(w >> 24) & 0xFF]) << 24) |
         (static_cast<uint32_t>(kSbox[(w >> 16) & 0xFF]) << 16) |
         (static_cast<uint32_t>(kSbox[(w >> 8) & 0xFF]) << 8) |
         static_cast<uint32_t>(kSbox[w & 0xFF]);
}

inline uint32_t RotWord(uint32_t w) {
  return (w << 8) | (w >> 24);
}

inline uint8_t Xtime(uint8_t x) {
  return static_cast<uint8_t>((x << 1) ^ ((x & 0x80) ? 0x1b : 0));
}

inline uint8_t Gmul(uint8_t a, uint8_t b) {
  uint8_t p = 0;
  for (int i = 0; i < 8; ++i) {
    if (b & 1) p ^= a;
    const uint8_t hi = static_cast<uint8_t>(a & 0x80);
    a = static_cast<uint8_t>(a << 1);
    if (hi) a ^= 0x1b;
    b = static_cast<uint8_t>(b >> 1);
  }
  return p;
}

void SubBytes(uint8_t state[16]) {
  for (int i = 0; i < 16; ++i) state[i] = kSbox[state[i]];
}

void ShiftRows(uint8_t state[16]) {
  uint8_t t;
  t = state[1];
  state[1] = state[5];
  state[5] = state[9];
  state[9] = state[13];
  state[13] = t;
  t = state[2];
  state[2] = state[10];
  state[10] = t;
  t = state[6];
  state[6] = state[14];
  state[14] = t;
  t = state[15];
  state[15] = state[11];
  state[11] = state[7];
  state[7] = state[3];
  state[3] = t;
}

void MixColumns(uint8_t state[16]) {
  for (int c = 0; c < 4; ++c) {
    uint8_t* col = state + c * 4;
    const uint8_t a0 = col[0];
    const uint8_t a1 = col[1];
    const uint8_t a2 = col[2];
    const uint8_t a3 = col[3];
    col[0] = static_cast<uint8_t>(Gmul(a0, 2) ^ Gmul(a1, 3) ^ a2 ^ a3);
    col[1] = static_cast<uint8_t>(a0 ^ Gmul(a1, 2) ^ Gmul(a2, 3) ^ a3);
    col[2] = static_cast<uint8_t>(a0 ^ a1 ^ Gmul(a2, 2) ^ Gmul(a3, 3));
    col[3] = static_cast<uint8_t>(Gmul(a0, 3) ^ a1 ^ a2 ^ Gmul(a3, 2));
  }
}

void AddRoundKey(uint8_t state[16], const uint32_t* rk) {
  for (int i = 0; i < 4; ++i) {
    const uint32_t w = rk[i];
    state[i * 4 + 0] ^= static_cast<uint8_t>((w >> 24) & 0xFF);
    state[i * 4 + 1] ^= static_cast<uint8_t>((w >> 16) & 0xFF);
    state[i * 4 + 2] ^= static_cast<uint8_t>((w >> 8) & 0xFF);
    state[i * 4 + 3] ^= static_cast<uint8_t>(w & 0xFF);
  }
}

}  // namespace

void Aes256KeyExpand(const uint8_t key[32], Aes256Key* out) {
  uint32_t temp;
  for (int i = 0; i < 8; ++i) {
    out->rk[i] = (static_cast<uint32_t>(key[4 * i]) << 24) |
                 (static_cast<uint32_t>(key[4 * i + 1]) << 16) |
                 (static_cast<uint32_t>(key[4 * i + 2]) << 8) |
                 static_cast<uint32_t>(key[4 * i + 3]);
  }
  for (int i = 8; i < 60; ++i) {
    temp = out->rk[i - 1];
    if (i % 8 == 0) {
      temp = SubWord(RotWord(temp)) ^ kRcon[i / 8 - 1];
    } else if (i % 8 == 4) {
      temp = SubWord(temp);
    }
    out->rk[i] = out->rk[i - 8] ^ temp;
  }
}

void Aes256EncryptBlock(const Aes256Key& key, const uint8_t in[16], uint8_t out[16]) {
  uint8_t state[16];
  std::memcpy(state, in, 16);
  AddRoundKey(state, key.rk);
  for (int round = 1; round < 14; ++round) {
    SubBytes(state);
    ShiftRows(state);
    MixColumns(state);
    AddRoundKey(state, key.rk + round * 4);
  }
  SubBytes(state);
  ShiftRows(state);
  AddRoundKey(state, key.rk + 56);
  std::memcpy(out, state, 16);
}

}  // namespace crypto
}  // namespace ebbackup
