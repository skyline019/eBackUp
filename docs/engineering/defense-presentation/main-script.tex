...
\input{fonts-gui.tex}
\input{macros.tex}
\input{chapters/code-snippets.tex}
...
\input{chapters/course-mapping.tex}
\begin{say}
...
\input{chapters/figures-sequence.tex}
\begin{say}
...
\input{chapters/figures-adapt-loop.tex}
\begin{say}
...
\end{say}
\input{chapters/tables-bench.tex}
...
\timing{—}{17--18.5\,min}
\input{chapters/figures-components.tex}
\begin{say}
...
\section{能力 Wave 弧（Wave A--T，ABI v29）}
\timing{—}{18--19\,min}
\begin{say}
\vtag{0.9.5} 在性能 Wave 5 之后，我们按 \filepath{CAPABILITY\_ROADMAP.md} 完成能力 sprint Wave A 到 Wave T：Wave B/E 写入 Manifest v5 与 Windows ADS/硬链/junction；Wave F/G 引入 \filepath{jobs.json} 与 GUI 作业管理；Wave I 默认 Hybrid CDC；Wave M 实现三路就地恢复；Wave N--O+ 补齐可达性、RPO、Windows 服务与 Workbench 多 Profile；Wave P/Q 接入 SQLite/Registry/VHDX 垂直插件；Wave R 提供可解释的智能排除建议；Wave T 闭合备份窗口与 deadline 自适应降级。

当前 C API ABI 为 v29，gtest 363 用例，ctest 三套件全绿。完整归档见 \filepath{docs/WAVE\_ARCHIVE.md}；工程技术细节见 \filepath{kernel-manual} 第 21 章；生态叙事见 \filepath{ecosystem-evolution} 第 13 章。
\end{say}

\section{架构与构件（技术版收尾）}
\timing{—}{19--20\,min}
\input{chapters/figures-architecture.tex}
\showshim
\begin{say}
...
\section{技术细节深挖（完整归档）}
\begin{say}
评委若追问 Windows 元数据、VSS、Manifest v5、**Pipeline 从零 / CDC 漂移 / L5 跃升**、三路恢复、EFS Tier 等\textbf{实现级}问题，完整模拟题与标准答法见同目录 \filepath{DEFENSE\_QA\_ARCHIVE.md}（Part A--M）及 \filepath{PIPELINE\_CDC\_DEEP\_DIVE.md}（Pipeline/CDC 专章）。本文档附录仅保留课程向快问快答；深挖备答以该 Markdown 归档为准。
\end{say}
