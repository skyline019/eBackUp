...
\textbf{内核提供：}\texttt{eb init/backup/restore/verify/compact/repo-stats/list-snapshots/prune}、daemon schedule/watch、稳定 C API（\filepath{include/ebbackup/eb\_backup.h}）。

\textbf{内核不提供：}云同步协议、多租户远程仓库、GUI Workbench、分布式 chunk 路由。集成方通过 C API 或 CLI 嵌入自有调度/UI。
\end{designbox}
