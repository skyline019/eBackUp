...
\begin{itemize}
  \item \textbf{不做}：多租户云仓库、S3 兼容 API、完整 SQLite 式查询引擎；
  \item \textbf{不做}：无测试的性能口号；未 opt-in 的语义变更；
  \item \textbf{做}：HCRBO/FastCDC、EbPack、Snapshot 时间旅行、RAR/Merkle 可验证、稳定 C API、daemon schedule/watch、桌面 Workbench（\filepath{gui/}）；
